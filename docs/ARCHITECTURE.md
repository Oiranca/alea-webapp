...
| `availability.ts` | Slot generation, conflict detection, `removable_top` surface logic |
| `security.ts` | `enforceMutationSecurity` (CSRF double-submit + origin + Fetch Metadata), `enforceRateLimit` (in-memory, trusts `x-forwarded-for` only when `x-real-ip` is inside `TRUSTED_PROXY_CIDRS` and the ingress rewrites the forwarding headers), `ensureCsrfCookie` |
